...
def test(model, data_loader, device, threshold=0.5):
    model.eval()
    ...
    with torch.no_grad():
        for idx, (images, image_ids) in tqdm(enumerate(data_loader),
            ...
            for i, output in enumerate(outputs):
                ...
                scores = output['scores'].cpu().numpy().tolist()
                labels = output['labels'].cpu().numpy().tolist()
                image_id = image_ids[i]  # Get image ID based on filename
...
# Load dataset
train_dataset = DigitDataset('nycu-hw2-data/train',
...
for epoch in range(epochs):
    ...
    torch.save(model.state_dict(), f'model7_epoch_{epoch+1}.pth')
